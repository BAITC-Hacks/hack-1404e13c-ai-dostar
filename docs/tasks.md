...
- [x] **2.4 Приемка** (тест дефицитов ждет 1.2) `tests/test_acceptance.py`, на реальных данных через `pipeline.run`:
...
- [ ] **2.5** `docs/methodology-forecast.md` — формулы и параметры для README (10–20 строк).

### Человек 1 — Спрос

До коммита 2.0 — изучить данные в ноутбуке или скрипте (контрольные SKU ниже), писать код в своих файлах.

- [ ] **1.1 Разовые заказы** `oneoffs.flag_oneoffs(sales_lines, params) -> SALES_FLAGGED`
...
- [ ] **1.3 Тесты** `tests/test_demand.py`: петля исключена; коробки не исключены; вколотая строка ×50 исключена; у SKU с дефицитными месяцами uplift > 0; `qty_regular` ≥ 0.
- [ ] **1.4 Отчет о разовых заказах** `oneoffs.report(sales_flagged) -> DataFrame` (дата, накладная, SKU, qty, типичный объем, причина) — для вкладки в UI.
- [ ] **1.5** `docs/methodology-demand.md` — алгоритм выбросов и дефицитов для README. Это обязательный пункт ТЗ, писать конкретно, с порогами.

### Человек 3 — Продукт

До 1:10 работает на `app/mock.py` — генератор DataFrame по `ORDER_LINES` (20–30 строк, оба поставщика, разные urgency). Потом одна строка меняется на `pipeline.run`.

- [ ] **3.1 Каркас Streamlit** `app/ui/app.py` (запуск: `.venv/bin/streamlit run app/ui/app.py`)
...
- [ ] **3.3 Экспорт** `app/export.py`: `to_xlsx(order_lines, supplier) -> bytes` и CSV, только утвержденные строки. Колонки: Код 1С, Артикул поставщика, Наименование, Ед., Количество, Поставщик, Склад, Обоснование. Экранировать значения, начинающиеся с `= + - @`.
- [ ] **3.4 Вкладка «Товар»** — выбор SKU: график по месяцам `qty_raw` / `qty_regular` / прогноз, маркеры разовых заказов и месяцев дефицита, `rationale` и разложение формулы.
- [ ] **3.5 Вкладка «Проверки»** — демо must-have: для выбранного SKU таблица «с фактором / без» по каждому `use_*` и по +N в пути. Это главный экран для жюри.
- [ ] **3.6 Вкладка «Сравнение с менеджером»** (SE) — `manager_baseline` против нашего расчета: ср. за 12 мес., свободный остаток, наша рекомендация, где расходимся сильнее всего.
- [ ] **3.7 Вкладка «Разовые заказы»** — `oneoffs.report()`.
- [ ] **3.8 README** (после 3:00): описание, запуск, структура, вставить `methodology-demand.md` и `methodology-forecast.md`, допущения из `architecture.md` §2.
- [x] **3.9** `app/copilot.py` (готово, сделал Человек 2; осталось добавить кнопки в UI) — «Объяснить подробнее» по строке через LLM: на вход только числа строки, fallback на `rationale`.
- [ ] **3.10** `docs/demo.md` — сценарий на 3 минуты: Петля LOOP → SKU с дефицитами → сезонный SKU → +в пути → утверждение и экспорт → сравнение с менеджером.
