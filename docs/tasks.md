...
Актуализация 2026-09-23: задачи человека 1 и продукта 3.1–3.10 реализованы. Пункты 2.2–2.3 закрыты коммитом `86e1d81`, регрессия критерия №4 проверяет итоговый заказ и проходит после смены оценки σ. Добавлено обучение ML по отдельному запросу пользователя: `app/ml`, временная проверка, артефакт модели и выбор метода в UI; [методика ML](methodology-ml.md). Оставшиеся вопросы данных и следующие эксперименты — в [плане завершения](completion-plan.md).
...
- [x] **3.9** `app/copilot.py` — обе кнопки подключены, сводка учитывает `final_qty`. LLM выбирает идентификаторы проверенных фактов; приложение формирует текст и числа из фиксированного каталога. Произвольный ответ, неполные данные или ошибка API → шаблон.
