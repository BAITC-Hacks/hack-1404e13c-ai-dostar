...
Этот отчёт фиксирует прогон ветки с `130bf37`. Позднее объединён `main` до `877cd34` и собран Linux-образ; актуальная проверка контейнера описана в [Docker](docker.md). В новом ассистенте LLM выбирает данные, текст ответа формируется шаблоном; ограничения старой проверки свободного текста ниже относятся к прежней версии.
