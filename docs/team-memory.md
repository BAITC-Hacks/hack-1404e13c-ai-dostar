...
- tests/test_ui.py проверяет граничные случаи на синтетической фикстуре без режима моков в продукте. tests/test_ui_real.py выполняет полный путь на реальных данных через настоящий pipeline. Оба изолируют утверждения и исключают платные API-вызовы. Сохранен scripts/ui_browser_smoke.py от человека 3 (Edge/Playwright); его повторный браузерный запуск в этой задаче не выполнялся. Сценарий — docs/demo.md.
