...
- 2026-09-23: По запросу пользователя в `ml/demand-forecast` объединён `main` до `5c5b3cc` (релизные заметки `59f4a27` и исправления ассистента). Сохранены новые ответы по шаблону, полный подсчёт до лимита, ограничение по поставщику и инвалидирование по `(calculation_id, lines.to_json())`; наши регрессии адаптированы к одному LLM-вызову выбора данных. Локальный полный pytest после объединения: **124 passed, 287.78 с, без пропусков**.
- Финальный Docker-образ после переноса панели ассистента наверх (`7a0c0c2`) прошёл полный pytest: **124 passed, 190.88 с, без пропусков**, под непривилегированным пользователем. Реальные ML и UI-тесты выполнялись внутри Linux, тесты подключались только для проверки и не входят в runtime-образ. Целевые проверки ассистента и реального UI на Windows: **21 passed, 74.30 с**. Оба браузерных режима финального образа успешны. Доказательства: `docs/verification/docker-summary.json`, `docker-pytest.xml`; итоговое объединение main до `877cd34` не меняло код приложения или проверенный Dockerfile.
- Добавлены `Dockerfile`, `.dockerignore`, `compose.yaml`, `requirements-docker.lock` (58 зафиксированных версий), `docs/docker.md`. Образ `ai-dostar:ml-demand` самостоятельно готовит реальные Excel и обучает ML в Linux; Windows joblib не копируется. Runtime: Python 3.14.7, sklearn 1.9.1, appuser UID 10001, healthcheck; исходные Excel, .env, .git и локальные утверждения в финальный образ не входят. Контейнер доступен на `http://127.0.0.1:8502`; volume `ai-dostar_approvals` сохраняется при пересоздании (проверено маркером, после проверки удалён). Внутри образа pip check и оба браузерных режима пройдены; ML-метрики совпали с прежними (−16.41% normalized MAE). Публикация в Docker registry не выполнялась.
...
- 2026-09-23: Post-release fixes on main (not included in v1.0.0). Fixed all four audit findings from `130bf37`: assistant uses LLM only for intent/filter selection and renders answers from calculated facts (removed unsafe free-text numeric guard); count precedes display limit; supplier scope applies to recommendations/lifecycle; UI invalidates saved answers after order edits or recalculation. Added regressions including mocked quantity invention and a real Streamlit AppTest for stale answers. Validation: `.venv/Scripts/python.exe -m pytest tests/test_assistant.py tests/test_copilot.py -q --basetemp=.pytest_cache/assistant-fix-0923b` -> 44 passed (7.41 s). Prior audit baseline: 110 passed excluding tests/test_ml.py (165 s). ML remains unverified locally because joblib/scikit-learn are absent; no ML code changed.
- 2026-09-23: **Итоговая проверка после интеграции `130bf37`: 119 passed, 153.32 с, 0 skip/xfail.** Edge заново прошёл статистику и ML, шесть вкладок, новую форму «Спросить ассистента» с таблицей оснований, карточку LOOP и mobile. Отчёт с ограничениями — `docs/system-verification.md`; JUnit, логи, скриншоты и SHA256 исходников — `docs/verification/`. Живой OpenAI API не проверен: ключ отсутствует; это явно указано, локальные сценарии и fallback работают.
- На этапе `130bf37` были добавлены защита от устаревшего свободного ответа и ошибок настройки LLM. После объединения `5c5b3cc` свободный текст/числовой фильтр заменены шаблонным ответом по вычисленным фактам; LLM выбирает только intent/filter. Инвалидирование после правки и пересчёта сохранено вместе с реальным AppTest; дополнительный API-вызов автоматически не выполняется.
- 2026-09-23: Проверка объединения `ml/demand-forecast` с `ddabe33`: **113 passed, 185.91 с**, без пропусков; Edge прошёл статистику и ML, все шесть вкладок, поиск и mobile. Реальный цикл `scripts/verify_real_system.py` пересобрал 8 таблиц, сверил 14 CSV из 12 книг, переобучил ML и сверил 2 698 будущих прогнозов с рабочим артефактом; проверил утверждение/CSV/XLSX обоих поставщиков в обоих методах в изолированном хранилище. Рабочие файлы не изменены. Notebook повторно выполнен, HTML проверен без сети. Материалы — `docs/verification/`. Внешний LLM не настроен и не проверен живым запросом.
- Исправлен воспроизведённый баг старой формы поиска; после перехода команды к свободным ответам защита адаптирована, как описано выше. Конвертер CSV по умолчанию ищет репозиторный `datasets/`. Старый IPv6 Streamlit остановлен; актуальный свежий сервер — `http://127.0.0.1:8501`. Рабочие утверждения не менялись; main не перезаписывался, результаты публикуются в `ml/demand-forecast`.



- (Superseded by the merge below: answers are rendered from facts.) В новой форме свободных ответов прежняя защита поиска адаптирована: ответ привязан к хешу текущих строк заказа; при редактировании скрывается, при пересчёте сбрасывается вместе с проверкой пар. Новый запрос автоматически не выполняется. Регрессионный AppTest работает на реальных данных. Ошибки настройки/создания клиента в `assistant.ask` теперь попадают в fallback; числа только из вопроса не считаются подтверждёнными фактами (добавлены 2 unit-регрессии). Смысловую правильность свободного текста числовая проверка не доказывает.
...
- 2026-09-23: Интегрирован `main` до `877cd34`, включая документацию Community Cloud и параллельную Docker-конфигурацию. Для этой ветки оставлен проверенный многоэтапный Dockerfile Python 3.14 и единственный `compose.yaml` (порт 8502, UID 10001, ML включён); дублирующий `docker-compose.yml` удалён, README согласован. Флаг `TRAIN_ML=0` здесь не поддерживается. Новых изменений приложения в `afe1024`/`877cd34` нет; повторная сборка после объединения документации не требуется.
...
- Docker: `docker compose up -d --build --wait` → http://127.0.0.1:8502 (first build installs deps, builds data, trains ML).
...
- 2026-09-23: Merged `origin/ml/demand-forecast` (ValiCoder: ML notebook + HTML, `docs/ml-visual-guide.md`, `docs/system-verification.md` with evidence in `docs/verification/`, `scripts/verify_real_system.py`, `requirements-notebooks.txt`) into `main` with `--no-ff`. Assistant conflict resolved in favour of Bekesh's `5c5b3cc`: the LLM only selects intent + filter, answers and quantities are rendered from calculated facts (a free-text numeric allowlist accepted real numbers from the wrong field, e.g. free_qty 40 as an order of 40). ValiCoder's «invented quantity» test kept, adapted to that behaviour. «Спросить ассистента» moved to a bordered panel above the tabs with example pills: `app.py` reserves `ask_slot` under the header and fills it after the tabs, so stale-answer invalidation sees this run's order edits. README: assistant section, versions note, documentation table.
...
- В ветке `ml/demand-forecast` добавлен `docs/ml-visual-guide.md`: схема обучения/прогноза, CPU и роли компонентов, ссылки на notebook, HTML и реальные графики. `notebooks/01_demand_model_walkthrough.py` — редактируемый источник; `scripts/run_ml_notebook.py` строит и выполняет `.ipynb`, сохраняет HTML и `docs/assets/ml/*`. Зависимости анализа отделены в `requirements-notebooks.txt`, ядро Jupyter ставится в `.venv`. Gym не используется: текущая задача — supervised regression, не RL.
- Проверено выполнение всех 20 ячеек на реальных parquet. Диагностический fit: цели до мая 2026 (65 513 примеров), проверка из мая на июнь–август (6 840), normalized MAE 0.981676 → 0.839353. Кривая всех 150 шагов, permutation importance на полной проверке с тремя перестановками, остатки и два заранее выбранных демо-SKU. Август IEK `130300792_` не проходит фильтр остатка и явно отмечен пропуском, не дорисован.
- Дополнительная проверка: 36 локальных Markdown-ссылок существуют; nbformat валиден, все 10 кодовых ячеек выполнены без ошибок. Edge открыл HTML с заблокированной сетью, все пять встроенных графиков загрузились. Независимый обход экспортированных CSV-деревьев воспроизвёл прогноз рабочего артефакта на 50 реальных примерах. Первоначальный замер fit: 1.074 с; при повторном прогоне параллельно с общей проверкой — 1.449 с, подготовка 56.907 с. Актуальные замеры — `diagnostics.json`; время зависит от нагрузки.
- Отдельный полный fit на 86 094 примерах воспроизвёл рабочие прогнозы на 1 000 фиксированных строках. CPU-замеры подготовки/fit, версии и SHA256 — `docs/assets/ml/diagnostics.json`, читаемая таблица — `run-summary.md`. Это новый замер, не восстановленный старый лог; рабочий `data/models/demand.joblib` не меняется. У модели **19 признаков, 150 деревьев, 4 350 узлов, 2 250 листьев**; все узлы экспортированы в CSV с расшифровкой категориальных разбиений. Полная августовская модель не оценивается на собственном обучении как на будущем.
- Ветка визуализации ответвлена от `b0bc121`; затем интегрированы `569ab56` (ассистент) и `ddabe33` (README), проверка — в Progress выше. Прежние 104 теста относятся к `b0bc121`; актуальный результат публикуется отдельно для объединённого кода.
