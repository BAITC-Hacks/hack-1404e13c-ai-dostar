...
- Docker-образ с этим объединением прошёл тот же полный pytest: **124 passed, 196.05 с, без пропусков**, под непривилегированным пользователем. Реальные ML и UI-тесты выполнялись внутри Linux, тесты подключались только для проверки и не входят в runtime-образ. Также интегрирован `7a0c0c2` с переносом панели ассистента наверх; целевые проверки ассистента и реального UI: **21 passed, 74.30 с**. Образ пересобран, выполняется повторная полная проверка.
