...
- Перед публикацией ветки получен `origin/main` с командным `569ab56` (ассистент заказов). Эта работа ответвлена от `b0bc121`, main не перезаписывается; новые изменения ассистента в ветку визуализации не включались. Продуктовый код текущей задачей не менялся, прежние 104 теста относятся к состоянию `b0bc121`, не являются проверкой нового командного ассистента.
