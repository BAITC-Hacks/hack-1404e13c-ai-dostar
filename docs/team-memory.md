...
- Исправлен воспроизведённый баг ассистента: результаты поиска обновляются при правке заказа без нового LLM-запроса; при пересчёте сбрасываются поиск/проверка пар. Добавлен реальный UI regression. Конвертер CSV по умолчанию ищет репозиторный `datasets/`. Старый IPv6 Streamlit остановлен; актуальный сервер — `http://127.0.0.1:8501`. После этой проверки получен новый `130bf37`; его интеграция и проверка выполняются следующим шагом.
