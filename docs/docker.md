...
Ветка `ml/demand-forecast` объединяет `main` до `877cd34`. Собран и запущен `ai-dostar:ml-demand`, Linux AMD64, Python 3.14.7, UID 10001. Идентификатор образа, хеши кода, строки таблиц и метаданные обученной модели: [docker-summary.json](verification/docker-summary.json).

- После исправлений ассистента: полный pytest на Windows — **124 passed**, внутри первого Linux-образа — **124 passed**.
- После переноса панели ассистента наверх: **21 passed** в целевых проверках ассистента и реального UI; оба браузерных прогона последнего образа успешны (статистика и ML, шесть вкладок, вопрос ассистенту, карточка товара, мобильный размер).
- Полный повторный pytest в последнем образе: **124 passed за 190.88 с, 0 пропусков** — [JUnit](verification/docker-pytest.xml).
- `pip check` успешен; работающий сервис `healthy`; хеши Python-кода в контейнере совпадают с исходниками. Сохранение volume при пересоздании проверено.
- Модель обучена на реальных данных: 86 094 примера, 248 875 исходных строк продаж. Внешний OpenAI API в этом прогоне не проверялся; локальный ассистент работает по правилам.

Полный pytest можно повторить внутри образа из PowerShell:

```powershell
docker run --rm --mount "type=bind,source=$($PWD.Path)\tests,target=/app/tests,readonly" ai-dostar:ml-demand python -m pytest -q --basetemp=/tmp/pytest
```
