...
def main():
    ...
    with sync_playwright() as p:
        ...
        panel.get_by_role("textbox", name="Опишите, что найти").fill("IEK заказ")
        panel.get_by_role("button", name="Найти", exact=True).click()
        expect(panel.get_by_text("Фильтр:", exact=False)).to_be_visible(timeout=30000)
