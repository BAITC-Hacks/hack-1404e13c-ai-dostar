...
def main():
    ...
    with sync_playwright() as p:
        ...
        panel.get_by_role("textbox", name="Ваш вопрос", exact=True).fill("IEK заказ")
        panel.get_by_role("button", name="Спросить", exact=True).click()
        expect(panel.get_by_text("Источник:", exact=False)).to_be_visible(timeout=30000)
        panel.get_by_text("На чем основан ответ:", exact=False).click()
        ...
        assert page.get_by_test_id("stException").count() == 0
        panel.get_by_text("Источник:", exact=False).scroll_into_view_if_needed()
